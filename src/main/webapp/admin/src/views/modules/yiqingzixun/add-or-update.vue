...
export default {
  ...
  methods: {
	...
	this.ruleForm.zixunneirong = this.ruleForm.zixunneirong.replace(reg,'../../../ssm1i23q/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yiqingzixun/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yiqingzixunCrossAddOrUpdateFlag = false;
                  this.parent.search();
