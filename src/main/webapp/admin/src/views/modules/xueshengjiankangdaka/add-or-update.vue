    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="有无咳嗽" prop="youwukesou">
          <el-select v-model="ruleForm.youwukesou" placeholder="请选择有无咳嗽">
            <el-option
...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `xueshengjiankangdaka/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xueshengjiankangdaka/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xueshengjiankangdakaCrossAddOrUpdateFlag = false;
                  this.parent.search();
