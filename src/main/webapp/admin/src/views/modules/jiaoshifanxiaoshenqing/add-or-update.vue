<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="返校日期" prop="fanxiaoriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.fanxiaoriqi" 
                type="date"
                placeholder="返校日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.fanxiaoriqi" label="返校日期" prop="fanxiaoriqi">
              <el-input v-model="ruleForm.fanxiaoriqi" 
                placeholder="返校日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="返校方式" prop="fanxiaofangshi">
          <el-input v-model="ruleForm.fanxiaofangshi" 
              placeholder="返校方式" clearable  :readonly="ro.fanxiaofangshi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="返校方式" prop="fanxiaofangshi">
              <el-input v-model="ruleForm.fanxiaofangshi" 
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="返校车次" prop="fanxiaocheci">
          <el-input v-model="ruleForm.fanxiaocheci" 
              placeholder="返校车次" clearable  :readonly="ro.fanxiaocheci"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="返校车次" prop="fanxiaocheci">
              <el-input v-model="ruleForm.fanxiaocheci" 
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="做在地风险等级" prop="zuozaidifengxiandengji">
          <el-select v-model="ruleForm.zuozaidifengxiandengji" placeholder="请选择做在地风险等级">
            <el-option
                v-for="(item,index) in zuozaidifengxiandengjiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="做在地风险等级" prop="zuozaidifengxiandengji">
	      <el-input v-model="ruleForm.zuozaidifengxiandengji"
                placeholder="做在地风险等级" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.hesuanjiancebaogao" label="核酸检测报告" prop="hesuanjiancebaogao">
          <file-upload
          tip="点击上传核酸检测报告"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.hesuanjiancebaogao" label="核酸检测报告" prop="hesuanjiancebaogao">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.hesuanjiancebaogao.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="返校后是否进行隔离" prop="fanxiaohoushifoujinxinggeli">
          <el-select v-model="ruleForm.fanxiaohoushifoujinxinggeli" placeholder="请选择返校后是否进行隔离">
            <el-option
                v-for="(item,index) in fanxiaohoushifoujinxinggeliOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="返校后是否进行隔离" prop="fanxiaohoushifoujinxinggeli">
	      <el-input v-model="ruleForm.fanxiaohoushifoujinxinggeli"
                placeholder="返校后是否进行隔离" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  data() {
	...
	jiatingzhuzhi : false,
	fanxiaoriqi : false,
	fanxiaofangshi : false,
	fanxiaocheci : false,
	zuozaidifengxiandengji : false,
	hesuanjiancebaogao : false,
	fanxiaohoushifoujinxinggeli : false,
	sfsh : false,
	shhf : false,
      },
      ruleForm: {
        ...
        fanxiaofangshi: '',
        fanxiaocheci: '',
        zuozaidifengxiandengji: '',
        hesuanjiancebaogao: '',
        fanxiaohoushifoujinxinggeli: '',
        shhf: '',
      },
          zuozaidifengxiandengjiOptions: [],
          fanxiaohoushifoujinxinggeliOptions: [],
      rules: {
          jiaoshigonghao: [
          ],
          jiaoshixingming: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jiatingzhuzhi = true;
          ...
          if(o=='fanxiaoriqi'){
            this.ruleForm.fanxiaoriqi = obj[o];
	    this.ro.fanxiaoriqi = true;
            continue;
          }
          if(o=='fanxiaofangshi'){
            this.ruleForm.fanxiaofangshi = obj[o];
	    this.ro.fanxiaofangshi = true;
            continue;
          }
          if(o=='fanxiaocheci'){
            this.ruleForm.fanxiaocheci = obj[o];
	    this.ro.fanxiaocheci = true;
            continue;
          }
          if(o=='zuozaidifengxiandengji'){
            this.ruleForm.zuozaidifengxiandengji = obj[o];
	    this.ro.zuozaidifengxiandengji = true;
            continue;
          }
	    ...
	    this.ro.hesuanjiancebaogao = true;
          ...
          if(o=='fanxiaohoushifoujinxinggeli'){
            this.ruleForm.fanxiaohoushifoujinxinggeli = obj[o];
	    this.ro.fanxiaohoushifoujinxinggeli = true;
            continue;
          }
        }
      }
      ...
      this.$http({
		...
		if(json.jiatingzhuzhi!=''&&json.jiatingzhuzhi){
          ...
          this.$message.error(data.msg);
            ...
            this.zuozaidifengxiandengjiOptions = "低风险,高风险".split(',')
            this.fanxiaohoushifoujinxinggeliOptions = "是,否".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `jiaoshifanxiaoshenqing/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jiaoshifanxiaoshenqing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiaoshifanxiaoshenqingCrossAddOrUpdateFlag = false;
                  this.parent.search();
